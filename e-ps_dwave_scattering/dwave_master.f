! THIS FILE COMPUTES D-WAVE PHASE SHIFTS FOR E-PS ELASTIC SCATTERING FOR
! BOTH SINGLET AND TRIPLET SPIN STATES WITHOUT THE 'MIXED SYMMETRY' TERM
! IN THE SHORT-RANGE EXPANSION OF THE WAVE FUNCTION.
!
! AUTHOR: WILLIAM MITCHELL (UNIVERSITY OF NORTH TEXAS) WORKING UNDER DR.
! SANDRA WARD QUINTANILLA
     ...
     1FILS(969,2,10),FILC(969,2,10)
      ...
      ! THE INPUT FILE IS PD.DATA, THE OUTPUT IS PD.TXT
  ...
  100         CONTINUE
   ...
   10         CONTINUE
              ...
              ! THE FOLLOWING COMPUTES THE CLC DIRECT-DIRECT MATRIX
              ! ELEMENT
              RECZ = 1.0D0/ZETA
              NN = NS
              CNST1 = -1.0D0
  ...
  130         CONTINUE
    ...
    7         CONTINUE
      ...
      SUBROUTINE COLUMN
              ! THIS SUBROUTINE COMPUTES ALL OF THE MIXED-RANGE ELEMENTS
              IMPLICIT REAL * 8 (A-H,O-Z)
   ...
   20         CONTINUE
    ...
    9         CONTINUE
     ...
     1             (3.0D0*COS1*AREC2*RREC2)
              ...
              S2 = (PI/32.0D0)*SQRT(AKK/(2.0D0*PI))*RREC2*POT13*SY
     ...
     1               (NB*EXZ-1.0D0)*CY1)-(1.0D0/3.0D0)*POT13*CY1*SHFNN
              ...
              C2 = (3.0D0*PI/32.0D0)*SQRT(AKK/(2.0D0*PI))*RREC2*FINT
              ...
              II = II+1
              IF (I.LE.N1F) THEN
                      ANG = SING*ANGP + TRIP*ANGM
              ELSE IF (I.GT.N1F) THEN
                      ANG = SING*ANGM + TRIP*ANGP
              END IF

              FICF = FICOEF(I)
              FILS2(II) = FILS2(II)+FICF*S2*ANG
              FILC2(II) = FILC2(II)+FICF*C2*ANG
   16         CONTINUE
   15         CONTINUE
    7         CONTINUE
    6         CONTINUE
...
!-------------FILS1 AND FILS2 ARE IN EQUIVALENCE
   ...
   11         CONTINUE
    ...
    9         CONTINUE
     ...
     1              R3**2+1.8D0*R2**2*R3**2)
              ...
              ! FIFI TERM
              PVE = 5.0D0/16.0D0*(9.0D0*R1**4+9.0D0*R2**4+4.0D0*R3**4+
     1              63.0D0/5.0D0*R1**2*R2**2-8.0D0*R1**2*R3**2-8.0D0*
     1              R2**2*R3**2+171.0D0/5.0D0*R1**2*R2**2*COS12**2-
     1              32.0D0*R1**3*R2*COS12-32.0D0*R1*R2**3*COS12+
     1              16.0D0*R1*R2*R3**2*COS12)

              PT = PW1+FIJCF(JA)*(PW2+PW3+PW4+PW5)

              DO 150 IA=I5,JA
              FILFI(IA,JA) = FILFI(IA,JA)+PT*FI(IA)
              FIFI(IA,JA) = FIFI(IA,JA)+FI(IA)*FIJCF(JA)*PVE
  150         CONTINUE
  100         CONTINUE
     ...
     1              R3**2-9.6D0*R2**2*R3**2)
              ...
              ! FIFI TERM
              PVE = 9.0D0/16.0D0*(R1**4+R2**4+R1**2*R2**2-3.0D0*
     1              R1**2*R2**2*COS12**2)
  ...
  300         CONTINUE
              ...
              !THESE ARE THE +L- AND -L+ TERMS, THEY SHOULD BE
              !EQUIVALENT, FIRST IS +L-
              IF (ISPIN.EQ.0) THEN
     ...
     1                      R3**2)
                      ...
                      ! FIFI TERM
                      PVE = 15.0D0/16.0D0*(R1**4-R2**4-8.0D0/5.0D0*R1**3
     1                      *R2*COS12+8.0D0/5.0D0*R1*R2**3*COS12)

                      PT = PW1+FIJCF(JA)*(PW2+PW3+PW5)+PW4

                      DO 250 IA=1,N1F
                      FILFI(IA,JA) = FILFI(IA,JA)+PT*FI(IA)
                      FIFI(IA,JA) = FIFI(IA,JA)+FI(IA)*FIJCF(JA)*
     1                              PVE
  250                 CONTINUE
  200                 CONTINUE
  ...
  400                 CONTINUE
    ...
    5         CONTINUE
      ...
      SUBROUTINE RMAT
              ! THIS SUBROUTINE CALCULATES THE MATRIX EQUATION AND OUR
              ! PHASE SHIFTS
